...
public class HttpDto implements Serializable
{
	...
	{
		this(url, new BaseRequest(), false);
	}

	public HttpDto(String url, BaseRequest model)
	{
		this(url, model, false);
	}

	// -------------- 终极实体 --------------
	public HttpDto(String url, BaseRequest model, boolean silence)
	{
		this.url = url;
		this.fullUrl = url;
		this.model = model;
	...
	public <T,R extends Request> R getRequest()
	{
		Request request;
		if(type == GET)
		{
	...
	public BaseRequest getModel()
	{
		return model;
	}

	public HttpDto setModel(BaseRequest model)
	{
		this.model = model;
		return this;
	}
